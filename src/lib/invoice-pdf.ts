...
export async function generateInvoicePDF(
	...
	doc.fontSize(12).fillColor(textColor).text(data.customerName, 50, 120);
	...
	if (data.customerAddress) {
		doc
			.fontSize(10)
			.fillColor(mutedColor)
			.text(data.customerAddress, 50, 155, { width: 200 });
	}
	...
	// Table header
	doc.fillColor(primaryColor).rect(tableLeft, tableTop, tableWidth, 25).fill();
	...
	for (const item of data.lineItems) {
		...
		if (data.lineItems.indexOf(item) % 2 === 0) {
			doc
				.fillColor("#F9FAFB")
				.rect(tableLeft, currentY - 5, tableWidth, 25)
				.fill();
		}
	...
	doc
		...
		.text(
			...
			totalsX + 80,
			currentY,
			{ align: "right", width: totalsWidth - 80 },
		);
	...
	doc
		...
		.fillColor(textColor)
		.text(formatCurrency(data.tax, data.currency), totalsX + 80, currentY, {
			align: "right",
	...
	doc.fontSize(10).fillColor(primaryColor).text("PAYMENT TERMS", 50, currentY);
	...
	if (data.notes) {
		currentY += 60;
		doc.fillColor(primaryColor).text("NOTES", 50, currentY);
		doc
			.fillColor(mutedColor)
