...
async function getRecentKnowledge(options: {
  ...
  const { organizationId, type = 'all', status, category, limit = 20 } = options
  const results: SearchResult[] = []

  // Fetch recent articles
  if (type === 'all' || type === 'article') {
    const articleConditions = [eq(knowledgeArticle.organizationId, organizationId)]
    if (status) articleConditions.push(eq(knowledgeArticle.status, status))
    if (category) articleConditions.push(eq(knowledgeArticle.category, category))
