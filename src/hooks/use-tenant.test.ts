...
describe("use-tenant utilities", () => {
	describe("buildTenantPath", () => {
		...
		it("should work with different tenant slugs", () => {
			expect(buildTenantPath("tenant1", "/app/support")).toBe(
				"/tenant1/app/support",
			);
			expect(buildTenantPath("tenant2", "/app/support")).toBe(
				"/tenant2/app/support",
			);
			expect(buildTenantPath("my-company", "/app/support")).toBe(
				"/my-company/app/support",
			);
		});
