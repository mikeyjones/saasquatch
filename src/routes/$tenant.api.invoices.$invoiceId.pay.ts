...
export const Route = createFileRoute(
	...
	server: {
		handlers: {
			/**
			 * POST /:tenant/api/invoices/:invoiceId/pay
					...
					if (inv.lineItems) {
						...
						for (const lineItem of lineItemsWithPlans) {
							...
							if (planDetails.length === 0) {
								console.warn(
									`Product plan not found: ${lineItem.productPlanId}`,
								);
								continue;
							}
							...
							if (pricingData.length > 0) {
								...
								const monthlyPricing = pricingData.find(
									(p) => p.interval === "monthly",
								);
								const yearlyPricing = pricingData.find(
									(p) => p.interval === "yearly",
								);
