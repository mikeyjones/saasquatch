...
import { Plus, Loader2, Package, Pencil, Trash2, DollarSign, Zap } from 'lucide-react'
...
function AddOnsPage() {
  ...
  const handleDelete = async (addOn: AvailableAddOn) => {
    ...
    try {
      const response = await fetch(`/api/tenant/${tenant}/product-catalog/add-ons?id=${addOn.id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
                  ...
                  {addOn.basePrice ? (
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className="text-sm text-gray-500">
                        ...
                        {addOn.basePrice.interval && `/${addOn.basePrice.interval}`}
                      </span>
                    </div>
                  ) : (
                    <span className="text-sm text-gray-500 italic">No pricing set</span>
                  )}
                </div>
