...
export const Route = createFileRoute(
	...
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/product-catalog/plans
			...
			GET: async ({ request, params }) => {
				try {
					...
					if (planIds.length > 0) {
						const boltOnPromises = planIds.map((planId) =>
							db
								...
								.innerJoin(
									productAddOn,
									eq(productPlanAddOn.productAddOnId, productAddOn.id),
								)
					...
					const addOnIds = [
						...new Set(allBoltOns.map((b) => b.productAddOnId)),
					];
			...
			/**
			 * POST /:tenant/api/product-catalog/plans
			...
			/**
			 * PUT /:tenant/api/product-catalog/plans
			...
			/**
			 * DELETE /:tenant/api/product-catalog/plans
