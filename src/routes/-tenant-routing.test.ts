...
describe("Multi-tenant route patterns", () => {
	...
	describe("URL generation for different tenants", () => {
		...
		it('should generate correct URLs for tenant "acme"', () => {
			const tenant = "acme";
			expect(generateUrl(tenantRoutePatterns.tickets, tenant)).toBe(
				"/acme/app/support/tickets",
			);
			expect(generateUrl(tenantRoutePatterns.members, tenant)).toBe(
				"/acme/app/support/members",
			);
		});
		...
		it('should generate correct URLs for tenant "globex"', () => {
			const tenant = "globex";
			expect(generateUrl(tenantRoutePatterns.tickets, tenant)).toBe(
				"/globex/app/support/tickets",
			);
			expect(generateUrl(tenantRoutePatterns.members, tenant)).toBe(
				"/globex/app/support/members",
			);
		});
...
describe("Tenant data isolation", () => {
	...
	it("each tenant should have separate data", () => {
		expect(mockTenantData.tenant1.organizationId).not.toBe(
			mockTenantData.tenant2.organizationId,
		);
	});
	...
	it("should query data by organizationId", () => {
		const getDataForOrg = (orgId: string) => {
			return (
				Object.values(mockTenantData).find((t) => t.organizationId === orgId)
					?.data ?? []
			);
		};
