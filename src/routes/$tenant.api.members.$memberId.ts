...
export const Route = createFileRoute("/$tenant/api/members/$memberId")({
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/members/:memberId
			...
			/**
			 * PUT /:tenant/api/members/:memberId
			...
			PUT: async ({ request, params }) => {
				try {
					...
					if (supportMember.length === 0) {
						return new Response(
							JSON.stringify({
								error: "Forbidden: Not a support staff member",
							}),
					...
					const { name, email, phone, title, role, status, notes } = body as {
						name?: string;
						email?: string;
						phone?: string;
						title?: string;
					...
					if (
						role !== undefined &&
						["owner", "admin", "user", "viewer"].includes(role)
					) {
							...
							if (existingMember.isOwner || existingMember.role === "owner") {
								...
								const ownerCount = await db
									...
									.where(
										and(
											eq(
												tenantUser.tenantOrganizationId,
												existingMember.tenantOrganizationId,
											),
								...
								if (ownerCount[0].count === 0) {
									return new Response(
										JSON.stringify({
											error:
												"Cannot change role: This is the last owner. At least one owner must remain.",
										}),
										{
											status: 400,
											headers: { "Content-Type": "application/json" },
					...
					if (
						status !== undefined &&
						["active", "suspended", "invited"].includes(status)
					) {
