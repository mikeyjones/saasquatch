...
export const Route = createFileRoute('/api/tenant/$tenant/product-catalog/products')({
  server: {
    handlers: {
      /**
       * GET /api/tenant/:tenant/product-catalog/products
              ...
              return {
                ...
                basePrice: basePricing
                  ? {
                      ...
                      perSeatAmount: basePricing.perSeatAmount ? basePricing.perSeatAmount / 100 : undefined,
                    }
                  : {
                      amount: 0,
                      currency: 'USD',
      ...
      /**
       * POST /api/tenant/:tenant/product-catalog/products
      ...
      POST: async ({ request, params }) => {
        try {
          ...
          const now = new Date()
          const productId = crypto.randomUUID().replace(/-/g, '').substring(0, 24)
      ...
      /**
       * PUT /api/tenant/:tenant/product-catalog/products
      ...
      PUT: async ({ request, params }) => {
        try {
          ...
          const existing = await db
            ...
            .from(productFamily)
            .where(
              and(eq(productFamily.id, id), eq(productFamily.organizationId, orgId))
            )
            .limit(1)
      ...
      /**
       * DELETE /api/tenant/:tenant/product-catalog/products
