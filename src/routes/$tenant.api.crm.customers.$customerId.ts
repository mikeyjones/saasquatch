...
export const Route = createFileRoute(
	...
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/crm/customers/:customerId
			...
			GET: async ({ request, params }) => {
				try {
					...
					// Fetch customer - support both ID and slug lookup
					...
					if (customer.length === 0) {
						customer = await db
							...
							.where(
								and(
									eq(tenantOrganization.slug, params.customerId),
									eq(tenantOrganization.organizationId, orgId),
								),
							)
							.limit(1);
					...
					const subscriptions = await db
						...
						.leftJoin(
							productFamily,
							eq(productPlan.productFamilyId, productFamily.id),
						)
						.where(
							and(
								eq(subscription.tenantOrganizationId, c.id),
					...
					const contacts = await db
						...
						.from(tenantUser)
						.where(eq(tenantUser.tenantOrganizationId, c.id))
					...
					const invoices = await db
						...
						.from(invoice)
						.where(eq(invoice.tenantOrganizationId, c.id))
					...
					const deals = await db
						...
						.where(eq(deal.tenantOrganizationId, c.id))
					...
					const quotes = await db
						...
						.where(
							and(
								eq(quote.tenantOrganizationId, c.id),
			...
			/**
			 * PUT /:tenant/api/crm/customers/:customerId
			...
			PUT: async ({ request, params }) => {
				try {
					...
					// Verify customer exists and belongs to this organization (support both ID and slug)
					...
					const parsedMetadata = customer.metadata
