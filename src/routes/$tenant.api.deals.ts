...
export const Route = createFileRoute("/$tenant/api/deals")({
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/deals
					...
					const contacts =
						dealIds.length > 0
							? await db
									...
									.innerJoin(
										tenantUser,
										eq(dealContact.tenantUserId, tenantUser.id),
									)
				...
				try {
					...
					if (!name || !tenantOrganizationId || !pipelineId || !stageId) {
						return new Response(
							JSON.stringify({
								error:
									"Missing required fields: name, tenantOrganizationId, pipelineId, stageId",
							}),
