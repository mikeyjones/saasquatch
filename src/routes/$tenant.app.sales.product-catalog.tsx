...
function ProductCatalogPage() {
	...
	const [selectedProductForPlan, setSelectedProductForPlan] =
	...
	const handleDeleteProduct = async (product: Product) => {
		if (
			!confirm(
				`Are you sure you want to delete "${product.name}"? This will also remove all associated plans. This action cannot be undone.`,
			)
		) {
			return;
		}
			...
			{!isLoading && products.length === 0 && (
				<div className="text-center py-12">
					...
					<p className="text-gray-500 mb-4 max-w-md mx-auto">
						Get started by creating your first product. You can then add pricing
						plans to each product.
					</p>
					<Button
						className="bg-indigo-500 hover:bg-indigo-600 text-white"
						onClick={handleCreateProduct}
