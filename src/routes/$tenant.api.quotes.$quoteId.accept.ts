 ...
 * - POST /:tenant/api/quotes/:quoteId/accept - Accept and convert
 *
 * This endpoint:
...
import { db } from "@/db";
import { quote, invoice, tenantOrganization, organization } from "@/db/schema";
...
export const Route = createFileRoute(
	...
	server: {
		handlers: {
			/**
			 * POST /:tenant/api/quotes/:quoteId/accept
					...
					if (issueDate) {
						...
						if (isNaN(issueDateObj.getTime())) {
							return new Response(
								JSON.stringify({
									error:
										"Invalid issueDate format. Expected ISO 8601 date string.",
								}),
								{
									status: 400,
									headers: { "Content-Type": "application/json" },
					...
					if (dueDate) {
						...
						if (isNaN(dueDateObj.getTime())) {
							return new Response(
								JSON.stringify({
									error:
										"Invalid dueDate format. Expected ISO 8601 date string.",
								}),
								{
									status: 400,
									headers: { "Content-Type": "application/json" },
