...
describe("Subscriptions API endpoint", () => {
	describe("GET /:tenant/api/subscriptions", () => {
		...
		describe("Response Shape", () => {
			...
			it("should include required subscription fields", () => {
				...
				expect(["active", "trial", "past_due", "canceled", "paused"]).toContain(
		...
		describe("MRR Calculation", () => {
			...
			it("should convert yearly pricing to monthly MRR", () => {
				...
				expect(yearlyCalculation.mrr).toBe(
					Math.round(yearlyCalculation.basePrice / 12),
				);
			});
			...
			it("should include per-seat pricing in MRR", () => {
				...
				expect(seatBasedCalculation.mrr).toBe(
					seatBasedCalculation.perSeatPrice * seatBasedCalculation.seats,
				);
			});
		});
	});

	describe("POST /:tenant/api/subscriptions", () => {
		describe("Authentication and Authorization", () => {
			it("should require authentication", () => {
		...
		describe("Duplicate Subscription Prevention", () => {
			it("should prevent creating second active subscription for same company", () => {
				const validation = {
					check:
						"subscription.tenantOrganizationId === tenantOrganizationId AND subscription.status === active",
					expectError: "Company already has an active subscription",
				};
				expect(validation.expectError).toBe(
			...
			it("should allow creating subscription if existing one is canceled", () => {
				const scenario = {
					...
					expectedResult:
						"success - canceled subscriptions do not block new ones",
				};
			...
			it("should allow creating subscription if existing one is paused", () => {
				const scenario = {
					...
					expectedResult:
						"success - paused subscriptions do not block new ones",
				};
			...
			it("should check for existing subscription before creating", () => {
				...
				expect(
					checkOrder.indexOf("check for existing active subscription"),
				).toBeLessThan(checkOrder.indexOf("create subscription"));
			});
		...
		describe("Subscription Number Generation", () => {
			...
			it("should increment subscription number from existing count", () => {
				...
				expect(generation.newNumber).toBe(
					`SUB-${generation.baseNumber + generation.existingCount}`,
				);
			});
		});
		...
		describe("Activity Tracking", () => {
			it("should create activity entry on subscription creation", () => {
				const activity = {
					...
					description:
						"Subscription SUB-1001 created for Acme Corp on Enterprise plan",
					userId: "user-uuid",
				};
...
describe("Subscription Detail API endpoint", () => {
	describe("GET /:tenant/api/subscriptions/:subscriptionId", () => {
		describe("Authentication and Authorization", () => {
			it("should require authentication", () => {
				const expectedResponse = { error: "Unauthorized" };
				expect(expectedResponse.error).toBe("Unauthorized");
			...
			it("should scope to organization", () => {
				const scopingRule = {
					filter:
						"subscription.organizationId === org.id AND subscription.id === params.subscriptionId",
				};
				expect(scopingRule.filter).toContain("organizationId");
	...
	describe("PUT /:tenant/api/subscriptions/:subscriptionId", () => {
	...
	describe("DELETE /:tenant/api/subscriptions/:subscriptionId", () => {
...
describe("Organization Scoping Security", () => {
	...
	it("should validate tenant organization belongs to current organization", () => {
		const securityRule = {
			...
			prevention:
				"Cannot create subscription for tenant from another organization",
		};
