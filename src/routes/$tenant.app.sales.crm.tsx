...
function CRMPage() {
  ...
  const [counts, setCounts] = useState({ all: 0, customers: 0, prospects: 0, inactive: 0 })
  ...
  const totals = useMemo(() => {
    const realizedValue = customers.reduce((sum, c) => sum + c.realizedValue, 0)
    const potentialValue = customers.reduce((sum, c) => sum + c.potentialValue, 0)
  ...
  return (
    <main className="flex-1 overflow-auto p-6">
      ...
      <CRMFilters
        ...
        industries={industries.length > 0 ? industries : ['Technology', 'Finance', 'Healthcare', 'Retail', 'Education', 'Manufacturing', 'Legal', 'Energy']}
      />
