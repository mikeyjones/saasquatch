...
function ProgramCard({ program, onManagePartners, onSettings }: ProgramCardProps) {
  const formatCurrency = (value: number): string => {
    return `$${value.toLocaleString()}`
  }
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-5">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <p className="text-xl font-semibold text-gray-900">{program.partners}</p>
        </div>
        <div className="bg-gray-50 rounded-lg p-3">
          <div className="flex items-center gap-2 text-gray-500 text-xs uppercase tracking-wide mb-1">
            <DollarSign size={14} />
          ...
          <p className="text-xl font-semibold text-gray-900">{program.commission}</p>
        </div>
      </div>
