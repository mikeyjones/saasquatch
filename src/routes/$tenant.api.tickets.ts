...
export const Route = createFileRoute("/$tenant/api/tickets")({
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/tickets
					...
					const triages =
						ticketIds.length > 0
							? await db
									...
									.where(
										or(
											...ticketIds.map((id) => eq(ticketAiTriage.ticketId, id)),
										),
									)
							: [];
