...
export const Route = createFileRoute("/$tenant/api/crm/customers")({
	server: {
		handlers: {
			/**
				...
				try {
					...
					const assignedUsers =
						assignedUserIds.length > 0
							? await db
									...
									.from(user)
									.where(or(...assignedUserIds.map((id) => eq(user.id, id))))
							: [];
					...
					const contactCounts = await db
						...
						.where(
							or(
								...tenantOrgIds.map((id) =>
									eq(tenantUser.tenantOrganizationId, id),
								),
							),
						)
					...
					const activitiesByTenantOrg = new Map<
						string,
					...
					for (const activity of recentActivities) {
						...
						const activities = activitiesByTenantOrg.get(
							activity.tenantOrganizationId,
						);
					...
					const activityUsers =
						activityUserIds.length > 0
							? await db
									.select({ id: user.id, name: user.name })
									.from(user)
									.where(or(...activityUserIds.map((id) => eq(user.id, id))))
							: [];
					const activityUserMap = new Map(
						activityUsers.map((u) => [u.id, u.name]),
					);
					...
					const customers = tenantOrgs.map((org) => {
						...
						const lastActivity =
							activities.length > 0
								...
								: dealData.deals.length > 0
									? dealData.deals.reduce(
											(latest, d) =>
												d.createdAt > latest ? d.createdAt : latest,
											dealData.deals[0].createdAt,
										)
									: org.updatedAt;
						...
						return {
							...
							importance: (org.importance || "normal") as
								| "low"
								| "normal"
								| "high"
								| "vip",
				...
				try {
					...
					if (createSubscription) {
						if (!subscriptionData?.productPlanId) {
							return new Response(
								JSON.stringify({
									error: "Product plan is required when creating subscription",
								}),
								{
									status: 400,
									headers: { "Content-Type": "application/json" },
					...
					if (createSubscription && subscriptionData) {
						...
						await db
							...
							.set({
								...
								subscriptionStatus:
									subStatus === "trial" ? "trialing" : subStatus,
								updatedAt: now,
							})
