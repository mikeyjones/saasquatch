...
import { db } from "@/db";
import { productAddOn, productAddOnPricing, organization } from "@/db/schema";
...
export const Route = createFileRoute(
	...
	server: {
		handlers: {
			/**
			 * GET /:tenant/api/product-catalog/add-ons
			...
			/**
			 * POST /:tenant/api/product-catalog/add-ons
			...
			POST: async ({ request, params }) => {
				try {
					...
					const { name, description, pricingModel, status, basePrice } =
						body as {
							name: string;
							description?: string;
			...
			/**
			 * PUT /:tenant/api/product-catalog/add-ons
			...
			PUT: async ({ request, params }) => {
				try {
					...
					// Parse request body
					const body = await request.json();
					const { id, name, description, pricingModel, status, basePrice } =
						body as {
							id: string;
							name?: string;
							description?: string;
					...
					const existing = await db
						...
						.where(
							and(
								eq(productAddOn.id, id),
								eq(productAddOn.organizationId, orgId),
							),
						)
						.limit(1);
			...
			/**
			 * DELETE /:tenant/api/product-catalog/add-ons
