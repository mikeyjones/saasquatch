...
import { db } from "@/db";
import { deal, dealActivity, pipelineStage, organization } from "@/db/schema";
...
export const Route = createFileRoute("/$tenant/api/deals/$dealId")({
	server: {
		handlers: {
			/**
			 * PUT /:tenant/api/deals/:dealId
			...
			PUT: async ({ request, params }) => {
				try {
					...
					const existingDeal = await db
						...
						.from(deal)
						.where(
							and(eq(deal.id, params.dealId), eq(deal.organizationId, orgId)),
						)
						.limit(1);
					...
					if (
						...
						assignedToUserId !== currentDeal.assignedToUserId
					) {
					...
					if (
						...
						manualScore !== currentDeal.manualScore
					) {
			...
			/**
			 * DELETE /:tenant/api/deals/:dealId
			 * Delete a deal
			 */
