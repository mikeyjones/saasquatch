...
export const Route = createFileRoute('/api/tenant/$tenant/coupons')({
  server: {
    handlers: {
      /**
       * GET /api/tenant/:tenant/coupons
      ...
      GET: async ({ request, params }) => {
        try {
          ...
          let filteredCoupons = coupons.map((c) => {
            ...
            if (c.status === 'active' && c.expiresAt && new Date(c.expiresAt) < new Date()) {
              actualStatus = 'expired'
            }
            if (c.status === 'active' && c.maxRedemptions && c.redemptionCount >= c.maxRedemptions) {
              actualStatus = 'expired'
            }
          ...
          return new Response(
            JSON.stringify({ coupons: filteredCoupons }),
      ...
      /**
       * POST /api/tenant/:tenant/coupons
       * Create a new coupon
       * Body:
      ...
      POST: async ({ request, params }) => {
        try {
          ...
          if (!discountType || !['percentage', 'fixed_amount', 'free_months', 'trial_extension'].includes(discountType)) {
            return new Response(
