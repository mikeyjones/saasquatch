...
describe('fetchMembers', () => {
	...
	it('should include search query in URL when provided', async () => {
		mockFetch()
			.mockResolvedValueOnce(mockFetchSuccess({ users: [] }))
	...
	it('should fetch open tickets count for each member', async () => {
		...
		mockFetch()
			.mockResolvedValueOnce(mockFetchSuccess({ users: mockMembers }))
			.mockResolvedValueOnce(mockFetchSuccess({ tickets: [{ id: 'ticket-1' }] })) // member-1 has 1 ticket
			.mockResolvedValueOnce(mockFetchSuccess({ tickets: [{ id: 'ticket-2' }, { id: 'ticket-3' }] })) // member-2 has 2 tickets
