...
describe('CreateContactDialog', () => {
	...
	describe('Render Behavior', () => {
		...
		it('should not render dialog when closed', () => {
			...
			expect(screen.queryByRole('heading', { name: /add new contact/i })).not.toBeInTheDocument()
		})
		...
		it('should show edit title in edit mode', async () => {
			...
			mockFetch().mockResolvedValueOnce(mockFetchSuccess({ contact: mockContact }))
	...
	describe('Form Submission', () => {
		it('should call API to create contact', async () => {
			...
			mockFetch().mockResolvedValueOnce(mockFetchSuccess({ contact: { id: 'contact-1' } }))
			...
			await waitFor(() => {
				expect(fetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/tenant/acme/crm/customers/customer-1/contacts'),
		...
		it('should call API to update contact in edit mode', async () => {
			...
			mockFetch()
				...
				.mockResolvedValueOnce(mockFetchSuccess({ contact: { ...mockContact, name: 'John Updated' } }))
