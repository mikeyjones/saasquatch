...
describe('ProductPlanDialog', () => {
	...
	describe('Render Behavior', () => {
		...
		it('should show edit title in edit mode', async () => {
			const mockPlan = {
				...
				basePrice: { amount: 99, currency: 'USD', interval: 'monthly' as const },
				regionalPricing: [],
				features: [],
				boltOns: [],
			}
		...
		it('should show dialog description in create mode', async () => {
			...
			expect(screen.getByText(/create a new subscription plan/i)).toBeInTheDocument()
		})
	})
	...
	describe('Regional Pricing', () => {
		...
		it('should show helper text when no regional pricing', async () => {
			...
			expect(screen.getByText(/add region-specific pricing/i)).toBeInTheDocument()
		})
	})
	...
	describe('Form Submission', () => {
		it('should call createPlan when form is submitted in create mode', async () => {
			...
			await waitFor(() => {
				expect(productsData.createPlan).toHaveBeenCalledWith('acme', expect.objectContaining({
					name: 'New Plan',
				}))
			})
		})
		...
		it('should call updatePlan when form is submitted in edit mode', async () => {
			...
			await waitFor(() => {
				expect(productsData.updatePlan).toHaveBeenCalledWith('acme', expect.objectContaining({
					id: 'plan-1',
				}))
			})
		})
	...
	describe('Bolt-Ons Integration', () => {
		it('should fetch add-ons when dialog opens', async () => {
			...
			await waitFor(() => {
				expect(productsData.fetchAddOns).toHaveBeenCalledWith('acme', { status: 'active' })
			})
		})
		...
		it('should show add-on selector when add-ons are available', async () => {
			...
			await waitFor(() => {
				expect(screen.getByText(/select add-on to include/i)).toBeInTheDocument()
			})
		})
	...
	describe('Edit Mode Population', () => {
		it('should populate form with plan data in edit mode', async () => {
			const mockPlan = {
				...
				basePrice: { amount: 199, currency: 'USD', interval: 'monthly' as const },
				regionalPricing: [
					{ region: 'GB', currency: 'GBP', amount: 159 },
				],
			...
			expect(screen.getByLabelText(/plan name/i)).toHaveValue('Enterprise Plan')
			expect(screen.getByLabelText(/description/i)).toHaveValue('For large teams')
			expect(screen.getByText('Unlimited users')).toBeInTheDocument()
