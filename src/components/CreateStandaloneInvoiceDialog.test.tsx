...
describe('CreateStandaloneInvoiceDialog', () => {
	...
	describe('Render Behavior', () => {
		...
		it('should not render dialog when closed', () => {
			...
			expect(screen.queryByRole('heading', { name: /create invoice/i })).not.toBeInTheDocument()
		})

		it('should show customer name in description', () => {
			render(<CreateStandaloneInvoiceDialog {...defaultProps} />)

			expect(screen.getByText(/create a standalone invoice for acme corp/i)).toBeInTheDocument()
		})
	})
	...
	describe('Line Items Management', () => {
		it('should render initial line item', () => {
			...
			expect(screen.getByPlaceholderText(/consulting services/i)).toBeInTheDocument()
		})
		...
		it('should add line item when Add Line is clicked', async () => {
			...
			const descriptionInputs = screen.getAllByPlaceholderText(/consulting services/i)
		...
		it('should remove line item when delete button is clicked', async () => {
			...
			const deleteButtons = screen.getAllByRole('button').filter(
				(btn) => btn.querySelector('svg.text-destructive') || btn.querySelector('[class*="destructive"]')
			)
	...
	describe('Form Submission', () => {
		it('should call API when form is submitted', async () => {
			const user = userEvent.setup()
			mockFetch().mockResolvedValueOnce(mockFetchSuccess({ invoice: { id: 'inv-1' } }))

			render(<CreateStandaloneInvoiceDialog {...defaultProps} />)

			// Fill in description
			const descriptionInput = screen.getByPlaceholderText(/consulting services/i)
