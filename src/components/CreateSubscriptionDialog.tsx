		...
		<Dialog open={open} onOpenChange={handleDialogClose}>
			<DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>
						Create a new subscription for a company. Each company can only have
				...
				<div className="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg text-sm flex items-start gap-3">
					...
					<div>
						<p className="font-medium">
							Subscription will be created in draft status
						</p>
						<p className="text-amber-700 mt-1">
							A draft invoice will be generated automatically. The subscription
							will become active when the invoice is marked as paid.
						</p>
					</div>
				</div>
				...
				<form
					onSubmit={async (e) => {
						...
						if (companyHasActiveSubscription) {
							setError(
								"This company already has an active subscription. Please cancel the existing subscription before creating a new one.",
							);
							return;
						}
					...
					<div className="space-y-2">
						...
						{companyHasActiveSubscription && (
							<p className="text-sm text-amber-600">
								⚠️ This company already has an active subscription. Cancel it
					...
					<div className="space-y-2">
						...
						{isLoadingData ? (
							...
							<div className="text-sm text-gray-500">
								No active product plans available. Create a plan first in the
								Product Catalog.
							</div>
						) : (
