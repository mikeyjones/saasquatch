...
const getNavItems = (tenant: string) => [
  { id: 'overview', label: 'Overview', icon: LayoutGrid, path: `/${tenant}/app/marketing` },
  { id: 'campaigns', label: 'Campaigns', icon: Megaphone, path: `/${tenant}/app/marketing/campaigns` },
  { id: 'journeys', label: 'Journeys', icon: GitBranch, path: `/${tenant}/app/marketing/journeys` },
  { id: 'audiences', label: 'Audiences', icon: Users, path: `/${tenant}/app/marketing/audiences` },
  { id: 'content', label: 'Content & CMS', icon: FileText, path: `/${tenant}/app/marketing/content` },
  { id: 'social', label: 'Social', icon: Share2, path: `/${tenant}/app/marketing/social` },
  { id: 'affiliates', label: 'Affiliates', icon: UserPlus, path: `/${tenant}/app/marketing/affiliates` },
  { id: 'agent', label: 'Agent Apollo', icon: Bot, path: '#' },
]

const departments = [
  { id: 'sales', label: 'Sales Department', icon: Briefcase, path: '/app/sales', color: 'bg-indigo-500' },
  { id: 'marketing', label: 'Marketing Dept', icon: Megaphone, path: '/app/marketing', color: 'bg-rose-500' },
...
export function MarketingSidebar() {
  ...
  const isActive = (path: string) => {
    if (path === `/${tenantSlug}/app/marketing`) {
      return currentPath === `/${tenantSlug}/app/marketing` || currentPath === `/${tenantSlug}/app/marketing/`
    }
      ...
      <div className="px-3 mb-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button type="button" className="w-full flex items-center justify-between px-3 py-2 bg-rose-600 hover:bg-rose-700 rounded-lg text-white text-sm transition-colors">
              <div className="flex items-center gap-2">
          ...
          <DropdownMenuContent align="start" className="w-52">
            ...
            {departments.map((dept) => {
              ...
              return (
                ...
                >
                  <div className={`w-5 h-5 ${dept.color} rounded flex items-center justify-center mr-2`}>
