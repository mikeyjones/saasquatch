...
describe("QuoteList", () => {
	...
	describe("Status Display", () => {
		...
		it("should display multiple quotes with different statuses", () => {
			render(
				<QuoteList quotes={[mockQuote, mockSentQuote, mockAcceptedQuote]} />,
			);
			expect(screen.getAllByText("Draft").length).toBeGreaterThan(0);
			expect(screen.getAllByText("Sent").length).toBeGreaterThan(0);
			expect(screen.getAllByText("Accepted").length).toBeGreaterThan(0);
	...
	describe("Action Buttons", () => {
		...
		it("should show Download PDF button when pdfPath exists", () => {
			...
			render(
				<QuoteList quotes={[mockSentQuote]} onDownloadPDF={onDownloadPDF} />,
			);
		...
		it("should disable Send button when isSending matches quote id", () => {
			...
			expect(sendButtons.some((btn) => btn.hasAttribute("disabled"))).toBe(
				true,
			);
		});
	});
	...
	describe("Multiple Quotes", () => {
		...
		it("should display correct customer for each quote", () => {
			const quotes = [
				...
				{
					...mockSentQuote,
					tenantOrganization: { id: "tenant-2", name: "Beta Inc" },
				},
			];
			render(<QuoteList quotes={quotes} />);
