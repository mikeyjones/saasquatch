...
export function OrganizationInvoiceHistory({
	invoices,
	onInvoiceUpdated,
}: OrganizationInvoiceHistoryProps) {
	...
	const statusCounts = {
		...
		draft: invoices.filter((inv) => inv.status.toLowerCase() === "draft")
			.length,
		final: invoices.filter((inv) => inv.status.toLowerCase() === "final")
			.length,
		overdue: invoices.filter((inv) => inv.status.toLowerCase() === "overdue")
			.length,
	};
	...
	return (
		<div>
			...
			<InvoiceDetailDialog
				...
				isFinalizing={
					selectedInvoice ? isFinalizing === selectedInvoice.id : false
				}
				isMarkingPaid={
					selectedInvoice ? isMarkingPaid === selectedInvoice.id : false
				}
