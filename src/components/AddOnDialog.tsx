...
export function AddOnDialog({
	...
	const [pricingModel, setPricingModel] = useState<"flat" | "seat" | "usage">(
		"flat",
	);
	...
	useEffect(() => {
		if (open) {
			...
			setPricingModel(
				(addOn?.pricingModel as "flat" | "seat" | "usage") || "flat",
			);
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const response = await fetch(url, {
				...
				credentials: "include",
				body: JSON.stringify(
					isEditMode ? { ...requestBody, id: addOn?.id } : requestBody,
				),
			});
			...
			if (!response.ok) {
				throw new Error(
					data.error || `Failed to ${isEditMode ? "update" : "create"} add-on`,
				);
			}
				...
				<form onSubmit={handleSubmit} className="space-y-5">
					...
					<div className="grid grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							...
							<Select
								value={pricingModel}
								onValueChange={(v) => setPricingModel(v as typeof pricingModel)}
							>
								<SelectTrigger>
					...
					<div className="space-y-3">
						...
						<p className="text-xs text-gray-500">
							Set the base price for this add-on. For usage-based pricing, this
							can be left empty.
						</p>
					</div>
