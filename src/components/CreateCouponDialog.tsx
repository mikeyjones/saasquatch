...
export function CreateCouponDialog({
  ...
  useEffect(() => {
    ...
    const loadCoupon = async () => {
      try {
        const response = await fetch(`/api/tenant/${tenant}/coupons/${couponId}`)
        if (response.ok) {
  ...
  useEffect(() => {
    ...
    const loadPlans = async () => {
      ...
      try {
        const response = await fetch(`/api/tenant/${tenant}/product-catalog/plans`)
        if (response.ok) {
          const data = await response.json()
          setPlans(data.plans || [])
        }
  ...
  const form = useAppForm({
    defaultValues: {
      code: '',
      discountType: 'percentage' as 'percentage' | 'fixed_amount' | 'free_months' | 'trial_extension',
      discountValue: 0,
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        console.log('discountType:', value.discountType, 'type:', typeof value.discountType)
        console.log('discountValue:', value.discountValue, 'type:', typeof value.discountValue)

        // Validate discount value
        if (value.discountValue === undefined || value.discountValue === null || value.discountValue === 0) {
        ...
        if (value.discountType === 'percentage' && (value.discountValue < 0 || value.discountValue > 100)) {
        ...
        console.error(`Error ${isEditMode ? 'updating' : 'creating'} coupon:`, err)
        setError(err instanceof Error ? err.message : `Failed to ${isEditMode ? 'update' : 'create'} coupon`)
      } finally {
        setIsSubmitting(false)
      }
    },
          ...
          <form.AppField name="code">
            {(field) => (
              <field.TextField
                ...
                disabled={isEditMode}
                helperText={isEditMode ? 'Code cannot be changed after creation' : 'Auto-generated if left empty'}
              />
            )}
          </form.AppField>
