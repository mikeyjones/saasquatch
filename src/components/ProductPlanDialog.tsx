...
export function ProductPlanDialog({
	...
	const form = useAppForm({
		defaultValues: {
			...
			pricingModel: (plan?.pricingModel || "flat") as
				| "flat"
				| "seat"
				| "usage"
				| "hybrid",
			...
			baseInterval: (plan?.basePrice?.interval || "monthly") as
				| "monthly"
				| "yearly",
		},
		validators: {
		...
		onSubmit: async ({ value }) => {
			...
			try {
				...
				const formattedBoltOns: BoltOnInput[] = selectedBoltOns.map(
					(bo, index) => ({
	...
	useEffect(() => {
		if (open) {
			...
			form.setFieldValue(
				"status",
				(plan?.status || "draft") as "active" | "draft" | "archived",
			);
			form.setFieldValue(
				"pricingModel",
				(plan?.pricingModel || "flat") as "flat" | "seat" | "usage" | "hybrid",
			);
			form.setFieldValue(
				"baseAmount",
				plan?.basePrice?.amount?.toString() || "0",
			);
			form.setFieldValue("baseCurrency", plan?.basePrice?.currency || "USD");
			form.setFieldValue(
				"baseInterval",
				(plan?.basePrice?.interval || "monthly") as "monthly" | "yearly",
			);
	...
	const removeBoltOn = (addOnId: string) => {
		setSelectedBoltOns(
			selectedBoltOns.filter((bo) => bo.productAddOnId !== addOnId),
		);
	};

	const updateBoltOnBillingType = (
		addOnId: string,
		billingType: "billed_with_main" | "consumable",
	) => {
		setSelectedBoltOns(
				...
				>
								...
								<form.Subscribe selector={(state) => state.values.baseAmount}>
									{(amount) => (
										<Input
											...
											value={amount}
											onChange={(e) =>
												form.setFieldValue("baseAmount", e.target.value)
											}
										/>
									)}
								</form.Subscribe>
								...
								{regionalPricing.map((rp, index) => (
									...
									>
										<select
											...
											value={rp.region}
											onChange={(e) =>
												updateRegionalPricing(index, "region", e.target.value)
											}
										>
										...
										<Input
											...
											value={rp.amount}
											onChange={(e) =>
												updateRegionalPricing(index, "amount", e.target.value)
											}
											className="w-28"
										/>
										<select
											...
											value={rp.currency}
											onChange={(e) =>
												updateRegionalPricing(index, "currency", e.target.value)
											}
										>
					...
					<div className="space-y-3">
												...
												<div>
													...
													{addOnDetails.basePrice && (
														<span className="text-xs text-gray-500 ml-2">
															${addOnDetails.basePrice.amount}/
															{addOnDetails.basePrice.interval || "mo"}
														</span>
													)}
												</div>
											</div>
											<div className="flex items-center gap-2">
												...
												<div className="flex items-center gap-1 bg-white rounded border border-gray-200 p-0.5">
													<button
														type="button"
														onClick={() =>
															updateBoltOnBillingType(
																boltOn.productAddOnId,
																"billed_with_main",
															)
														}
						...
						{selectedBoltOns.length === 0 &&
							...
							!isLoadingAddOns && (
								<p className="text-xs text-gray-500">
									No add-ons available. Create add-ons first to include them as
									bolt-ons.
								</p>
							)}
						{selectedBoltOns.length === 0 && availableAddOns.length > 0 && (
							<p className="text-xs text-gray-500">
								Add bolt-ons that customers can optionally purchase with this
								plan. Choose between recurring (billed with subscription) or
