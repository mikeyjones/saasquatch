...
const getNavItems = (tenant: string) => [
	{
		...
		label: "Overview",
		icon: LayoutGrid,
		path: `/${tenant}/app/support`,
	},
	{
		id: "tickets",
		label: "Tickets",
		icon: MessageSquare,
		path: `/${tenant}/app/support/tickets`,
	},
	{
		id: "members",
		label: "Members & Orgs",
		icon: Users,
		path: `/${tenant}/app/support/members`,
	},
	{
		id: "knowledge",
		label: "Knowledge & Playbooks",
		icon: BookOpen,
		path: `/${tenant}/app/support/knowledge`,
	},
...
export function SupportSidebar() {
	...
	const isActive = (path: string) => {
		if (path === `/${tenantSlug}/app/support`) {
			return (
				currentPath === `/${tenantSlug}/app/support` ||
				currentPath === `/${tenantSlug}/app/support/`
			);
		}
			...
			<div className="px-3 mb-2">
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<button
							type="button"
							className="w-full flex items-center justify-between px-3 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-white text-sm transition-colors"
						>
							<div className="flex items-center gap-2">
