...
vi.mock('./PipelineColumn', () => ({
	PipelineColumn: ({ stage, deals, totalPotential }: { stage: { id: string; name: string }; deals: unknown[]; totalPotential: number }) => (
...
describe('PipelineKanban', () => {
	...
	describe('Render Behavior', () => {
		it('should render pipeline columns after loading', async () => {
			...
			await waitFor(() => {
				expect(screen.getByTestId('pipeline-column-stage-1')).toBeInTheDocument()
			})
	...
	describe('Pipeline Selection', () => {
		it('should call onPipelineChange when pipeline is selected', async () => {
			...
			await waitFor(() => {
				expect(mockOnPipelineChange).toHaveBeenCalledWith(mockPipelines.pipelines[0])
			})
		})
		...
		it('should fetch deals when pipeline is selected', async () => {
			...
			await waitFor(() => {
				expect(fetch).toHaveBeenCalledWith(
					expect.stringContaining('/api/tenant/acme/deals?pipelineId=pipeline-1')
				)
			})
		})
	...
	describe('Stage Ordering', () => {
		it('should render stages in order', async () => {
			...
			const stageNames = screen.getAllByTestId('stage-name').map((el) => el.textContent)
