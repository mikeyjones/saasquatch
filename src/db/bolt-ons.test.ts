...
describe('Bolt-On Data Model', () => {
  it('should follow the pattern: ProductPlan <-> ProductPlanAddOn <-> ProductAddOn', () => {
    ...
    expect(relationships.productPlanAddOn.additionalData).toContain('billingType')
  })
...
describe('Bolt-On Display Ordering', () => {
  ...
  it('should handle reordering bolt-ons', () => {
    ...
    const sorted = [...reordered].sort((a, b) => a.displayOrder - b.displayOrder)
...
describe('Bolt-On Business Logic', () => {
  ...
  it('should not include consumable bolt-ons in MRR calculation', () => {
    ...
    const mrrBoltOns = boltOns.filter((b) => b.billingType === 'billed_with_main')
  ...
  it('should track consumable usage separately', () => {
    ...
    const overageUsage = consumableBoltOn.usageTracking.currentPeriodUsage - freeUsage
  ...
  it('should filter bolt-ons by billing type', () => {
    ...
    const recurring = planBoltOns.filter((b) => b.billingType === 'billed_with_main')
    const consumable = planBoltOns.filter((b) => b.billingType === 'consumable')
...
describe('Bolt-On Configuration Scenarios', () => {
  it('should support typical SaaS bolt-on configurations', () => {
    ...
    expect(typicalBoltOns.filter((b) => b.billingType === 'billed_with_main').length).toBe(3)
    expect(typicalBoltOns.filter((b) => b.billingType === 'consumable').length).toBe(2)
  })
...
describe('Bolt-On Type Definitions', () => {
  ...
  it('should match BoltOnInput interface structure', () => {
    ...
    expect(['billed_with_main', 'consumable']).toContain(boltOnInput.billingType)
