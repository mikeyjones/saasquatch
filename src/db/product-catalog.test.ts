...
describe("Product Catalog Data Model", () => {
	...
	it("should support bolt-ons as a many-to-many relationship", () => {
		...
		expect(boltOnRelationship.productPlanAddOn.billingTypes).toContain(
			"billed_with_main",
		);
		expect(boltOnRelationship.productPlanAddOn.billingTypes).toContain(
			"consumable",
		);
		expect(boltOnRelationship.productPlanAddOn.additionalFields).toContain(
			"billingType",
		);
	});
...
describe("Pricing Model Support", () => {
	...
	it("should support regional pricing", () => {
		...
		expect(regionalPricing.find((p) => p.region === "GB")?.currency).toBe(
			"GBP",
		);
...
describe("Product Feature Flags", () => {
	...
	it("should support typical SaaS feature flags", () => {
		...
		expect(
			typicalFlags.find((f) => f.flagKey === "api_access")?.flagValue,
		).toBe("true");
	});
});
...
describe("Currency Handling", () => {
	it("should store amounts in cents for precision", () => {
		const testPrices = [
			{ dollars: 29.0, cents: 2900 },
			{ dollars: 99.99, cents: 9999 },
			{ dollars: 149.0, cents: 14900 },
			{ dollars: 299.0, cents: 29900 },
		];
